...
def evaluateQuestion(question, link):
    if question:
        webbrowser.open(ZOOM_LINK_FORMAT.format(link))
        sys.exit()
    elif question is None:
        pass
    else:
        pyperclip.copy(ZOOM_LINK_FORMAT.format(link))
        sys.exit()
...
def createOptionsWindow(subjects_dictionary_list):
    ...
    selected_name = tkinter.StringVar(options_window)
    ...
    options_message = tkinter.Label(
        options_window,
        text = MSGBOX_OPTIONS
    )

    options_menu = tkinter.OptionMenu(
        options_window,
        selected_name,
        "Seleccionar",
        *list_of_options,
